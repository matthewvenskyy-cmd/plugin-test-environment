...
`npm.cmd run validate:config` checks scenario paths, exported scenario names and `run` functions, expected-failure regexes, duplicate or overlong `spawnBot` usernames, and literal command references to unknown or overlong player names.
