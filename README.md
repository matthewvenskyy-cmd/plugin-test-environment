...
Add `--dry-run` to `smoke`, `test`, or `scenarios` to see the selected plugin jars and scenario count without downloading Paper, building jars, or starting the server:

```powershell
node src/harness.js scenarios --plugin=CorePlugin --area=CorePlugin --dry-run
```
...
Manual scenarios are skipped by default and can be run with `--scenario=<name>`. Expected-failure scenarios are useful for regressions the environment can already detect but the plugin has not fixed yet. Every expected failure requires a `failurePattern` matching the known bug so earlier or unrelated errors still fail the suite. The suite also fails if an expected-failure scenario unexpectedly starts passing, which is the cue to remove the `expectedFailure` marker.

`npm.cmd run validate:config` checks top-level config shape, timeout and server port values, project entries, scenario entries, scenario paths, scenario files missing from the default config, duplicate scenario registrations, exported scenario names and `run` functions, expected-failure regexes, duplicate or overlong `spawnBot` usernames, literal command references to unknown or overlong player names, BCT/Corebreaker scenario helper usage, manual dropped-item cleanup that should use `clearDroppedItems`, manual BCT item/display cleanup that should use `clearBctArtifacts`, local chat, block, or entity polling helpers, and local player-health or attribute parsers that should use their shared helpers.

Scenario modules receive `ctx.waitForCondition(predicate, { timeoutMs, intervalMs, label })` for arbitrary polling, `ctx.waitForInventory(predicate, timeoutMs)` for the primary bot inventory, and `ctx.commandUntil(command, pattern, { timeoutMs, intervalMs, label })` for command output that should be pattern-matched instead of delayed by a fixed sleep. A scenario config object can set `timeoutMs` to override the global scenario timeout for deliberately long multi-bot workflows. Shared scenario helpers also export `waitForCondition`, `waitForChat`, `waitForNoChat`, `waitForVehicle`, `waitForPlayerPassengerState`, `waitForWindowSlot`, and `waitForServerBlock` for bot, mount-state, inventory-window, and server polling utilities, `serverCommandSucceeds`, `applyServerDamage`, `applyServerItemReplace`, `giveClassItem`, `serverRecipeExists`, `serverBlockIs`, `serverEntityExists`, `serverPlayerIsPassengerOf`, and `selectedItemHasNoDamage` for scoreboard-backed command, entity-relation, and boolean state, `isRocketlytraWithCharges` and `rocketlytraRecipe` for FireworksElytra item and client-recipe setup, `queryPlayerHealth` and `queryPlayerAttribute` for server-confirmed numeric player state, `digUntilServerBlock` for retrying a real Mineflayer dig until Paper confirms its result, `clearDroppedItems` for generic dropped-item cleanup, `clearBctArtifacts` for BCT item/display cleanup, `queryBctDisplayCount`, `countBctItemsNear`, `waitForBctDisplayCount`, and `waitForBctItemsNear` for BCT display/item accounting, `assertNoBctLeak` and `assertBctStateStable` for snapshot and observation-window cross-plugin leak assertions, and `assertWindowExcludesItemStable` for detecting delayed forbidden-item insertion into custom containers.
