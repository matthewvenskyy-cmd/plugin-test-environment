import { countItemsByName, displayText, waitForInventoryItem } from "./helpers.js";

export const name = "Rocketlytra craft preserves custom elytra name";

export async function run(ctx) {
  const { assert, bot, command, wait } = ctx;

  await command("clear ScenarioBot", 250);
  await command("gamemode survival ScenarioBot", 250);
  await command("recipe give ScenarioBot fireworkselytraplugin:rocketlytra_2", 500);
  await command(`give ScenarioBot minecraft:elytra[minecraft:custom_name='{"text":"Storm Wings","italic":false}'] 1`, 500);
  await command("give ScenarioBot minecraft:firework_rocket 2", 500);
  await waitForInventoryItem(bot, isCustomNamedElytra, "custom-named elytra");
  await waitForInventoryItem(bot, (item) => item?.name === "firework_rocket" && item.count >= 2, "firework rockets");

  await bot.craft(rocketlytraRecipe(bot, 2), 1, null);
  await wait(750);

  const rocketlytra = await waitForInventoryItem(bot, isCustomNamedRocketlytra(2), "custom-named Rocketlytra with 2 charges");
  const text = displayText(rocketlytra);
  assert(rocketlytra.name === "elytra", `crafted item should stay an elytra, got ${rocketlytra.name}`);
  assert(text.includes("Storm Wings"), `Rocketlytra should preserve the custom elytra name; item=${text}`);
  assert(text.includes("Rocketlytra charges") && text.includes("2"), `Rocketlytra should gain two charges; item=${text}`);
  assert(countItemsByName(bot, "firework_rocket") === 0, "crafting should consume exactly the two firework rockets");
  await command("gamemode creative ScenarioBot", 250);
}

function isCustomNamedElytra(item) {
  return item?.name === "elytra" && displayText(item).includes("Storm Wings");
}

function isCustomNamedRocketlytra(charges) {
  return (item) => {
    if (!isCustomNamedElytra(item)) return false;
    const text = displayText(item);
    return text.includes("Rocketlytra charges") && text.includes(String(charges));
  };
}

function rocketlytraRecipe(bot, fireworks) {
  const elytra = bot.registry.itemsByName.elytra.id;
  const firework = bot.registry.itemsByName.firework_rocket.id;
  const ingredient = (id) => ({ id, metadata: null, count: 1 });
  return {
    result: ingredient(elytra),
    ingredients: [ingredient(elytra), ...Array.from({ length: fireworks }, () => ingredient(firework))],
    delta: [
      { id: elytra, metadata: null, count: 0 },
      { id: firework, metadata: null, count: -fireworks }
    ],
    requiresTable: false
  };
}
