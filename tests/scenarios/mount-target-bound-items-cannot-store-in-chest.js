...
export async function run(ctx) {
  ...
  try {
    ...
    await waitForPlayerPassengerState(
      ...
      "MtTgtChest",
      true,
      "mounted target chest-storage attachment"
    );
    ...
    assert(
      await serverPlayerIsPassengerOf(ctx, "MtTgtChestR", "MtTgtChest"),
      "target bound-item chest denials should leave the rider attached"
    );

    const unmounted = await waitForChat(rider, () => rider.chat("/unmount"), /dismounted/i);
    assert(unmounted, "rider should dismount cleanly after target chest-storage denials");
    await waitForPlayerPassengerState(
      ctx,
      "MtTgtChestR",
      "MtTgtChest",
      false,
      "mounted target chest-storage detachment"
    );
  } finally {
    rider.chat("/unmount");
    await wait(500);
